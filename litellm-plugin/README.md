...
- **Anthropic Claude Max (`claude-*`)**: Keeps the required Claude Agent SDK identity first and anchors the last two cacheable conversation turns with rolling `cache_control: ephemeral` markers. The native TypeScript proxy applies the same two-breakpoint policy, skips thinking blocks, and respects Anthropic's four-marker ceiling. Also normalizes temperature for extended thinking and ensures `max_tokens > budget_tokens`.
